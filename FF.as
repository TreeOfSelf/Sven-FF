// Friendly fire plugin — NPC grenades only (monster_handgrenade)
// Author: Sebastian (patched for NPC-only monster_handgrenade support)
...
// Damage values for explosives — NPC-only: monster_handgrenade
dictionary ExplosiveDamges =
{
    { "monster_handgrenade", 100 }  // NPC hand grenade (match player grenade damage if desired)
};
...
// Keep player weapon hooks (they won't track NPC grenades) — track code left minimal
HookReturnCode WeaponPrimaryAttack( CBasePlayer@ pPlayer, CBasePlayerWeapon@ pWeapon )
{
    ...
    // We do NOT track player grenades for NPC-only mode.
    return HOOK_CONTINUE;
}
...
{
    // If we are not picking up a satchel or not player, ignore
...
// AddClassToTrackEntities now only accepts entities whose owner is an NPC (player ally).
// This version records only "monster_handgrenade" (but can be extended easily).
    ...
    while( ( @foundEntity = g_EntityFuncs.FindEntityByClassname( foundEntity, ClassName ) ) !is null ){
        if (trackedEntities.find(foundEntity.entindex()) == -1) {
            ...
            if (ownerHandle.IsValid()) {
                @ownerEntity = ownerHandle.GetEntity();
            }

            // Only track when the owner is a friendly NPC (player ally).
            // NPC grenades typically belong to NPCs (IsPlayerAlly()).
...
void TrackEntities()
{
    ...
    // NPC grenades only
    AddClassToTrackEntities("monster_handgrenade", "monster_handgrenade");
    ...
    for (int i = int(trackedEntities.length()) - 1; i >= 0; --i) {
        ...
        if (edict !is null) {
            ...
            // Entity has gone (exploded). Use last known pos to create explosion effect
            ...
            if (ExplosiveDamges.exists(entityType)) {
                ...
                // NPC grenades should use blast damage (unless you want different)
